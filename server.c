...
#include <sys/ipc.h>
#include <sys/shm.h>
// maximum 100 clients can connect with server
...
{
  char buf[200];
  // the u option is to update the existing tar file with new or modified files
  sprintf(buf, "tar -uvf %s %s", tar, filePath);
  // execute the command in os's command shell
  system(buf);
}
...
int main(int argc, char *argv[])
{
  ...
  int cnt = 0;                      // variable to store number of connected clients
  int server_socket, client_socket; // file descriptors for client and server socket
  ...
  while (1)
  {
    ...
    ++cnt; // incrementing number of connected clients

    // handling requests by mirror if number of client is >=7 or <= 12 or even number
    if ((cnt >= 7 && cnt <= 12) || (cnt > 12 && cnt % 2 == 0))
    {
      snprintf(response, 10, "mirror_f");
      send(client_socket, response, 10, 0);
      continue;
    }

    // handling requests by server if number of client is 1 to 6  or odd number
    printf("Client connected\n");
    snprintf(response, 10, "server_f");
    send(client_socket, response, 10, 0);

    // fork a new child process to serve the client exclusively
    ...
    {
      close(server_socket); // Closing parent socket
